var videoController = {
	...
	addVideoToScreen : function(videoObjs) {
		var that = this;		
		$.each(videoObjs, function(index, video) {
			var newVideoElement = that.uiElements.videoCardTemplate.clone()
					.attr('id', video.firebaseId);

			newVideoElement.click(function() {
				// the user has clicked on the video... let's play it, or pause
				// it depending on state
	...
	updateVideoOnScreen : function(firebaseId, videoObj) {
		var videoElement =  $('#' + videoId);//getElementForVideo(firebaseId);
		if (!videoObj) {
			return;
		}
	...
	getSignedUrls : function(videoObjs, callback) {
		...
		if (videoObjs) {
			...
			$.post(getSignedUrl, JSON.stringify(objectMap), function(data,
					status) {
				if (status === 'success') {					
