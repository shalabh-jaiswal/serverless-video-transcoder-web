...
var uploadController = {	data : {
	...
	wireEvents : function() {
		...
		this.uiElements.uploadButton.on('change', function(result) {
			...
			var requestDocumentUrl = that.data.config.apiBaseUrl
					+ '/s3-policy-document?filename=' + encodeURI(file.name);
	...
	upload : function(file, data, that) {
		...
		fd.append('policy', data.encodedPolicy)
	...
	progress : function() {
		...
		xhr.upload.onprogress = function(evt) {
			...
			$('#upload-progress').find('.progress-bar').css('width',
					percentage + '%');
		};
		return xhr;
	}
